...
//   const dispatch = useDispatch();
  const { sales, loading, error, totalPages } = useSelector(({sales}) => sales);
...
//     navigation.navigate('SaleDetails', { saleId: id }); 
//   };

  if (loading) return <Text>Loading...</Text>;
  if (error) return <Text>Error: {error}</Text>;

  console.log("Rendering with animals:", sales.map((sale) => sale.id));
